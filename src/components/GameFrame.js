...
export default class GameFrameComponent extends Component {
  ...
  componentDidMount = () => {
    AsyncStorage.getItem('@maxScore')
      .then((maxScore) =>
        this.setState({...DEFAULT_STATE, maxScore: JSON.parse(maxScore)}),
      )
      .catch((error) => {
        console.log(error);
  ...
  _popRandomActor = () => {
    ...
    if (!this.actors[randomIndex].isPopping) {
      ...
      const actorType =
        Math.random() < 0.1
          ? (this.setState({bossPopped: bossPopped + 1}), ActorType.BOSS)
          : Math.random() < 0.2
          ? (this.setState({boarPopped: boarPopped + 1}), ActorType.BOAR)
          : Math.random() < 0.1
          ? ActorType.BIKE
          : (this.setState({hunterPopped: hunterPopped + 1}), ActorType.HUNTER);
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        {this.state.start && (
          <StartComponent
            onStart={this._resume}
            onRules={() => this.setState({rules: true})}
            onCredits={() => this.setState({credits: true})}
          />
        )}
        {this.state.rules && (
          <RulesComponent onBack={() => this.setState({rules: false})} />
        )}
        {this.state.credits && (
          <CreditsComponent onBack={() => this.setState({credits: false})} />
        )}
        {this.state.gameover && (
          <GameOverComponent
            onReset={this._reset}
        ...
        {this.state.paused && (
          <PauseComponent onReset={this._reset} onResume={this._resume} />
        )}
        {this.state.leave && (
          <LeaveComponent onResume={this._resume} onLeave={this._reset} />
        )}
      </View>
    );
  }
