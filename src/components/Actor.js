...
export default class ActorComponent extends Component {
  ...
  huntersmash(isBoss) {
    this.setState(
      {pointsText: isBoss ? '+10' : '+1', pointsTextColor: 'green'},
  ...
  whackABoar() {
    this.setState(
      {pointsText: '-10', pointsTextColor: 'red'},
      this._startAnimation(),
    );
  ...
  _getApparitionTime() {
    return this.poppingActorType === ActorType.BOSS
      ? 0
      : 1000 - Math.min(Math.floor((this.props.level - 1) / 2), 10);
  }
  ...
  render() {
    ...
    return (
      <View style={{flex: 1}}>
        ...
        <Animated.Text
          onPress={() => this.whack()}
          style={[styles.absoluteContainer, styles.scoreText, animatedStyle]}>
